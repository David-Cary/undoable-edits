...
exports.UndoableJSONPatch = exports.DEFAULT_JSON_PATCH_OPS_TO_ACTIONS = exports.parseJSONPatchPathStep = exports.parseJSONPatchPath = void 0;
...
function parseJSONPatchPath(path) {
    if (typeof path === 'string') {
        ...
        if (steps[0] === '')
            steps.shift();
        var decoded = steps.map(function (step) { return parseJSONPatchPathStep(step); });
        return decoded;
    }
    return path;
}
...
/**
  ...
  * @returns {CommonKey}
  */
function parseJSONPatchPathStep(value) {
    var swapped = value.replace(/~0/g, '~').replace(/~1/g, '/');
    var index = Number(swapped);
    return isNaN(index) ? swapped : index;
}
exports.parseJSONPatchPathStep = parseJSONPatchPathStep;
